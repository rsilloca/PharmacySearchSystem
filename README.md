...
## Table of Contents
   ...
   * [Fury - Angular 9+ Material Design Admin Template](#fury---angular-6+-material-design-admin-template)
...
| `src`                 | Contains all Angular 9+ Typescript code, assets and basically everything the end user will have access to. |
...
Now that we have installed all prerequisites it's time to start developing our app. Angular-CLI offers a lot of assistance and allows you to generate basically all Angular 9+-Components there are. *(In a smart way!)*

To generate our first `component` we simply open up a terminal and navigate in our Angular 9+-App. Now we simply run `ng g component client` and we get a new component in `/src/app/client` with the following files:
...
These were the first steps with the awesome Angular-CLI and Angular 9+. Following we are going to get more in detail about developing with Angular-CLI and using the power of Angular 8+ right.
...
Receiving data from a server to process or display it in Angular 9+ can be quite tricky with the new observables and syntax. Here you'll find an example how you can do this.
...
* [RxJS](//github.com/ReactiveX/rxjs) - Angular 9+ Dependency - A new approach to how data can be handled in Observables.
